...
static NexButton bBack = NexButton(en_WinId_OEMLogin , 1, "b5");
static NexButton bProceed = NexButton(en_WinId_OEMLogin, 7 , "b1");
static NexText tPassword = NexText(en_WinId_OEMLogin, 4 , "t1");
...
{
	BeepBuzzer();
	if(0 == strncmp(objstcSettings.arr_COEMEditPassword , g_arrEnteredPassword , MAX_PASSWORD_LENGTH))
	{
		ChangeWindowPage(en_WinId_OEMSettings , (enWindowID)NULL);
	}
	else
	{
		enWindowID NextWindow = stcScreenNavigation.CurrentWindowId;
		if(enkeyOk == ShowMainPopUp("OEM Settings","Invalid Credentials", true))
		{
...
void HandlerbPassword(void *ptr)
{
	openKeyBoard(en_AlphaKeyboard , g_arrEnteredPassword , sizeof(g_arrEnteredPassword) , false ,
			"Enter OEM Settings Password" , g_arrEnteredPassword , MAX_PASSWORD_LENGTH);
	BeepBuzzer();
}
